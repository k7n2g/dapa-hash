...
use dapa_hash::v1::{dapa_hash, ScratchPad};
...
fn bench_zero_input(c: &mut Criterion) {
    ...
    let input = [0u8; 200];
    c.bench_function("v1::zero_input", |b| b.iter(|| dapa_hash(&mut input.clone(), &mut scratch_pad)));
}
...
fn bench_fixed_input(c: &mut Criterion) {
    ...
    c.bench_function("v1::fixed_input", |b| b.iter(|| dapa_hash(&mut input.clone(), &mut scratch_pad)));
}
