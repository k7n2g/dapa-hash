...
use dapa_hash::v2::{dapa_hash, ScratchPad};
...
fn bench_zero_input(c: &mut Criterion) {
    ...
    let input = [0u8; 112];
    c.bench_function("v2::zero_input", |b| b.iter(|| dapa_hash(&input, &mut scratch_pad)));
}

fn bench_fixed_input(c: &mut Criterion) {
    let mut scratch_pad = ScratchPad::default();
    c.bench_function("v2::fixed_input", |b| b.iter(|| dapa_hash(FIXED_INPUT, &mut scratch_pad)));
}
