...
pub fn dapa_hash(input: &[u8], scratch_pad: &mut ScratchPad, #[cfg(feature = "tracker")] distribution: &mut OpsTracker) -> Result<Hash, Error> {
    // stage 1
...
mod tests {
    ...
    fn test_reused_scratchpad() {
        ...
        // Do a first hash
        let expected_hash = dapa_hash(&input, &mut scratch_pad, #[cfg(feature = "tracker")] &mut OpsTracker::new(MEMORY_SIZE)).unwrap();

        // Do a second hash with dirty scratch pad but same input
        let hash = dapa_hash(&input, &mut scratch_pad, #[cfg(feature = "tracker")] &mut OpsTracker::new(MEMORY_SIZE)).unwrap();
    ...
    fn test_zero_hash() {
        ...
        let hash = dapa_hash(&mut input, &mut scratch_pad, #[cfg(feature = "tracker")] &mut OpsTracker::new(MEMORY_SIZE)).unwrap();
        let expected_hash = [
    ...
    fn test_distribution() {
        ...
        for _ in 0..ITERATIONS {
            OsRng.fill_bytes(&mut input);
            let _ = dapa_hash(&input, &mut scratch_pad, &mut distribution).unwrap();
        }
