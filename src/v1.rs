...
pub fn dapa_hash(input: &mut [u8; BYTES_ARRAY_INPUT], scratch_pad: &mut ScratchPad) -> Result<Hash, Error> {
...
mod tests {
    ...
    fn test_input(input: &mut [u8; BYTES_ARRAY_INPUT], expected_hash: Hash) {
        ...
        let hash = dapa_hash(input, &mut scratch_pad).unwrap();
    ...
    fn test_scratch_pad() {
        ...
        let hash = dapa_hash(input.as_mut_slice().unwrap(), &mut scratch_pad).unwrap();
        let expected_hash = [
            0x0e, 0xbb, 0xbd, 0x8a, 0x31, 0xed, 0xad, 0xfe, 0x09, 0x8f, 0x2d, 0x77, 0x0d, 0x84,
            0xb7, 0x19, 0x58, 0x86, 0x75, 0xab, 0x88, 0xa0, 0xa1, 0x70, 0x67, 0xd0, 0x0a, 0x8f,
            0x36, 0x18, 0x22, 0x65,
